...
for (let example in exampleCoverage) {
  ...
  if ( missingTests || missingKeys || missingAttrs ) {
    let coverageInfo = 'EXAMPLE ' + example + ':\n';

    if (existingTestIds === missingTests) {
      coverageInfo += '  * has no regression tests. Please add tests.\n';
      examplesWithNoTests += 1;
    }
    else if (missingTests) {
      coverageInfo += '  * is missing regression tests. Missing testIds include:\n';
    ...
    if (missingKeys) {
      coverageInfo += '  * has rows in the "Keyboard Support" table(s) without data-test-ids.\n    Please add data-test-ids to rows:\n';
    ...
    if (missingAttrs) {
      coverageInfo += '  * has rows in the "Attribues" table(s) without data-test-ids.\n    Please add data-test-ids to rows:\n';
